...
temp=`curl -X GET -H "Accept: application/json" "https://meteotest.ch/actions/mdxConnector/mdx/ortswetter?value=7550&name=Scuol&type=zip" 2>/dev/null | awk -F":" '{print $3 }'|awk -F "," '{ print $1 }'`
echo $temp
